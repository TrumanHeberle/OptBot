...
class DecisionTree:
    def __init__(self, root, decision_list, brancher, pruner):
        ...
        self.pruner = lambda node: pruner(node.value)
    def __str__(self):
    ...
    def ideal_path(self):
        ...
        return max(self.top, key=self.pruner).path()
    def ideal_key(self):
        """Returns the first key of the ideal traversal path."""
        return max(self.top, key=self.pruner).initial_branch().key
